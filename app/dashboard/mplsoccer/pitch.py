...
class Pitch(BasePitchPlot):
    ...
    def _validate_pad(self):
        ...
        if abs(min(self.pad_left, 0) + min(self.pad_right, 0)) >= self.dim.length:
            raise ValueError(
                "pad_left/pad_right too negative for pitch length")
    ...
    def _set_extent(self):
        ...
        if self.half:
            ...
            # do not want clipped values if half
        ...
        # kdeplot
        self.kde_clip = ((self.dim.left, self.dim.right),
    ...
    def _draw_arc(self, ax, x, y, width, height, theta1, theta2, **kwargs):
        arc = patches.Arc((x, y), width, height,
            ...
            if i % 2 == 0:
                if ((self.extent[0] <= self.dim.stripe_locations[i] <= self.extent[1]) or
                    ...
                    pitch_color[self.grass_stripe_start: self.grass_stripe_end, start: end] = \
                        pitch_color[self.grass_stripe_start: self.grass_stripe_end,
                                    start: end] + 2
        return pitch_color
...
class VerticalPitch(BasePitchPlot):
    ...
    def _validate_pad(self):
        ...
        if abs(min(self.pad_bottom, 0) + min(self.pad_top, 0)) >= self.dim.length:
            raise ValueError(
                "pad_bottom/pad_top too negative for pitch length")
    ...
    def _set_extent(self):
        ...
        if self.half:
            extent[2] = extent[2] - \
                min(self.pad_bottom, self.dim.pitch_length/2)
        ...
        # kdeplot
        self.kde_clip = ((self.dim.top, self.dim.bottom),
    ...
    def _draw_arc(self, ax, x, y, width, height, theta1, theta2, **kwargs):
        arc = patches.Arc((y, x), height, width,
                          theta1=theta1 + 90, theta2=theta2 + 90, **kwargs)
        ax.add_patch(arc)
            ...
            if i % 2 == 0:
                if ((self.extent[2] <= self.dim.stripe_locations[i] <= self.extent[3]) or
                    ...
                    pitch_color[start: end, self.grass_stripe_start: self.grass_stripe_end] = \
                        pitch_color[start: end,
                                    self.grass_stripe_start: self.grass_stripe_end] + 2
        return pitch_color
