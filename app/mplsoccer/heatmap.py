...
def bin_statistic(x, y, values=None, dim=None, statistic='count',
    ...
    if (values is None) & (statistic != 'count'):
        raise ValueError(
            "values on which to calculate the statistic are missing")
    if standardized:
    ...
    cx, cy = np.meshgrid(
        x_edge[:-1] + 0.5 * np.diff(x_edge), y_edge[:-1] + 0.5 * np.diff(y_edge))
...
def bin_statistic_sonar(x, y, angle, values=None, dim=None, statistic='count',
    ...
    if not len(bins) == 3:
        raise ValueError(
            "bins should be either an int, [int, int, int] or [array, array, array]")
    ...
    if standardized:
        ...
        if dim.invert_y:
            pitch_range = [[dim.left, dim.right], [
                dim.top, dim.bottom], [0, 2 * np.pi]]
            y = dim.bottom - y  # for inverted axis flip the coordinates
        else:
            pitch_range = [[dim.left, dim.right], [
                dim.bottom, dim.top], [0, 2 * np.pi]]
...
def sonar(stats_length, xindex=0, yindex=0,
    ...
    **kwargs : All other keyword arguments are passed on to matplotlib.axes.Axes.bar.
    ...
    if not isinstance(ax, PolarAxes):
        raise TypeError(
            'The ax argument must be of type matplotlib.projections.polar.PolarAxes.')
    if stats_color is not None and cmap is None:
        raise ValueError(
            "You must supply a cmap for varying the color using stats_color.")
    if stats_color is None and cmap is not None:
        raise ValueError(
            "You must supply a stats_color for varying the color using a cmap.")
    ...
    if cmap is not None:
        ...
        return ax.bar(stats_length['angle_grid'],
                      np.nan_to_num(stats_length['statistic'])[
            yindex, xindex, :],
...
def bin_statistic_positional(x, y, values=None, dim=None, positional='full',
    ...
    if positional == 'full':
        ...
        raise ValueError(
            "positional must be one of 'full', 'vertical' or 'horizontal'")
...
def heatmap_positional(stats, ax=None, vertical=False, **kwargs):
    ...
    **kwargs : All other keyword arguments are passed on to matplotlib.axes.Axes.pcolormesh.
    ...
    vmax = kwargs.pop('vmax', np.nanmax(
        [np.nanmax(stat['statistic']) for stat in stats]))
    vmin = kwargs.pop('vmin', np.nanmin(
        [np.nanmin(stat['statistic']) for stat in stats]))

    mesh_list = []
    for bin_stat in stats:
        mesh = heatmap(bin_stat, vmin=vmin, vmax=vmax,
                       ax=ax, vertical=vertical, **kwargs)
