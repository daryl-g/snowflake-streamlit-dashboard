    ...
    **kwargs : All other keyword arguments are passed on to matplotlib.collections.LineCollection.
    ...
    if transparent:
        cmap = create_transparent_cmap(
            color, cmap, n_segments, alpha_start, alpha_end)
    ...
    Legend.update_default_handler_map(
        {LineCollection: line_collection_handler})
...
def _create_segments(xstart, ystart, xend, yend, n_segments=100, multi_segment=False):
    if multi_segment:
        ...
        points = np.concatenate(
            [points, np.expand_dims(points[:, -1, :], 1)], axis=1)
        ...
        segments = np.transpose(
            np.array([[xstart, ystart], [xend, yend]]), (2, 0, 1))
    return segments
...
def _lines_no_cmap(xstart, ystart, xend, yend, lw=None, color=None, ax=None,
    ...
    line_collection = LineCollection(
        segments, color=color, linewidth=lw, snap=False, **kwargs)
...
def _lines_cmap(xstart, ystart, xend, yend, lw=None, cmap=None, ax=None,
    ...
    if reverse_cmap:
        cmap = cmap.reversed()
    line_collection = LineCollection(
        segments, cmap=cmap, linewidth=lw, snap=False, **kwargs)
    line_collection = ax.add_collection(line_collection)
    extent = ax.get_ylim()
...
class HandlerLines(HandlerLineCollection):
    ...
    def __init__(self, invert_y=False, first_lw=False, use_cmap=False,
        ...
        HandlerLineCollection.__init__(
            self, marker_pad=marker_pad, numpoints=numpoints, **kw)
