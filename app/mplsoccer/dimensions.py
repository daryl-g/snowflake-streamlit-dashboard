...
valid = ['statsbomb', 'tracab', 'opta', 'wyscout', 'uefa',
         ...
         'impect']
size_varies = ['tracab', 'metricasports',
               'custom', 'skillcorner', 'secondspectrum']
...
class BaseDims:
    ...
    def stripes(self):
        ...
        stripe_pen_area = (self.penalty_area_length -
                           self.six_yard_length) / 2.
        stripe_other = (self.length - 2 * self.six_yard_length -
                        6 * stripe_pen_area) / 10.
    ...
    def penalty_box_dims(self):
        ...
        self.six_yard_top = self.center_width + \
            (neg_if_inverted * self.six_yard_width)
        self.goal_bottom = self.center_width - \
            (neg_if_inverted * self.goal_width)
    ...
    def create_positions_five_per_line(self):
        ...
        x_flip = np.where(self.origin_center,
                          self.center_length - x, self.right - x)
        x_half_flip = np.where(
            self.origin_center, self.center_length - x_half, self.right - x_half)
        y_flip = np.where(self.origin_center, self.center_width -
                          y, max(self.top, self.bottom) - y)
    ...
    def create_positions_four_per_line(self):
        ...
        idx = [18, 1, 13, 25, 37, 2, 14, 26, 38,
               3, 15, 27, 39, 4, 16, 28, 40, 17, 29]
    ...
    def create_positions_four_per_line_ss(self):
        ...
        idx = [21, 1, 15, 29, 43, 2, 16, 30, 44,
               3, 17, 31, 45, 4, 18, 32, 46, 20, 34]
...
class MetricasportsDims(BaseDims):
    ...
    def __post_init__(self):
        ...
        self.six_yard_length = round(
            self.six_yard_length / self.pitch_length, 4)
        self.penalty_area_width = round(
            self.penalty_area_width / self.pitch_width, 4)
        self.penalty_area_length = round(
            self.penalty_area_length / self.pitch_length, 4)
        self.penalty_spot_distance = round(
            self.penalty_spot_distance / self.pitch_length, 4)
        self.penalty_left = self.penalty_spot_distance
...
class ScaleCenterDims(BaseDims):
    ...
    def __post_init__(self):
        ...
        self.six_yard_width = round(
            self.six_yard_width / self.pitch_width * self.width, 4)
        self.six_yard_length = round(
            self.six_yard_length / self.pitch_length * self.length, 4)
        self.penalty_area_width = round(
            self.penalty_area_width / self.pitch_width * self.width, 4)
        self.penalty_area_length = round(self.penalty_area_length /
                                         self.pitch_length * self.length, 4)
        self.goal_width = round(
            self.goal_width / self.pitch_width * self.width, 4)
        self.goal_length = round(
            self.goal_length / self.pitch_length * self.length, 4)
        self.penalty_area_left = self.left + self.penalty_area_length
        self.six_yard_left = self.left + self.six_yard_length
