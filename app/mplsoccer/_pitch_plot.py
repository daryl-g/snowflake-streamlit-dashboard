...
class BasePitchPlot(BasePitch):
    ...
    def _reflect_2d(self, x, y, standardized=False):
        ...
        if standardized:
            ...
            x_limits, y_limits = [self.dim.left, self.dim.right], [
                self.dim.bottom, self.dim.top]
        reflected_data_x = np.r_[
            x, 2 * x_limits[0] - x, 2 * x_limits[1] - x, x, x]
        reflected_data_y = np.r_[y, y, y, 2 *
                                 y_limits[0] - y, 2 * y_limits[1] - y]
    ...
    def hexbin(self, x, y, ax=None, **kwargs):
        ...
        **kwargs : All other keyword arguments are passed on to matplotlib.axes.Axes.hexbin.
        ...
        hexbin = ax.hexbin(x, y, mincnt=mincnt,
                           gridsize=gridsize, extent=extent, **kwargs)
    ...
    def voronoi(self, x, y, teams):
        ...
        reflect_x, reflect_y = self._reflect_2d(
            x, y, standardized=standardized)
        ...
        for region in regions:
            ...
            verts[:, 0] = np.clip(
                verts[:, 0], a_min=extent[0], a_max=extent[1])
            verts[:, 1] = np.clip(
                verts[:, 1], a_min=extent[2], a_max=extent[3])
    ...
    def flow(self, xstart, ystart, xend, yend, bins=(5, 4), arrow_type='same', arrow_length=5,
        ...
        **kwargs : All other keyword arguments are passed on to matplotlib.axes.Axes.quiver.
        ...
        if self.dim.invert_y and not standardized:
            endy = bs_angle['cy'] - \
                (np.sin(bs_angle['statistic']) * new_d)  # invert_y
        else:
